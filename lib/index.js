...
const TileRegion = require('./abstractions/TileRegion')
...
module.exports = async (input, {
  ...
  const mapSize = tileSize * tileLength * (resolution / 72)
  const mapRect = [0, 0, mapSize, mapSize]

  const tileRegion = new TileRegion(region, { tileSize, tileLength, resolution })

  const canvas = createCanvas(tileRegion.width, tileRegion.height)
  const ctx = canvas.getContext('2d')
  ctx.quality = quality
  ctx.antialias = antialias
  ctx.translate(-tileRegion.offset[0], -tileRegion.offset[1])
  ...
  const files = await tile(canvas, {
    tiles: [tileRegion.rectWidth, tileRegion.rectHeight],
    tileSize,
    verbose,
    antialias,
    quality,
    ...
    compression,
    skipEmpty,
    filePattern: (i, j) => path.join(output, filename(i + tileRegion.rect[0], j + tileRegion.rect[1], zoom) + '.png')
  })
  ...
  function transform ({ feature, bbox, lastFoundName }) {
    ...
    feature = project(feature, aabb.transform(mapRect, { precision, padding }))
  ...
  function renderRasters (feature) {
    ...
    const rendered = []
    const scale = aabb.computeScale(mapRect, { padding })
    const transformation = aabb.transform(mapRect, { padding, precision })
